...
**The config block above is version-specific, and on opencode 1.17.11 it is what breaks the run.** There, *any* `permission` key in `OPENCODE_CONFIG` hangs at `message=init` with a 0-byte output file — the full block, and a deny-only `{"edit": "deny", "write": "deny", "patch": "deny"}` subset alike, with no `evaluated permission` line ever logged. 1.17.11 allows `bash`/`read`/`grep`/`glob` by default (the session log shows only `question`/`plan_enter`/`plan_exit` denied), so the working invocation there is **no `OPENCODE_CONFIG` at all**.

**`external_directory` is not in that list, and the session log's silence about it is a trap.** On `add-expiring-urls` a 1.17.11 run with no config logged exactly those three denials and then died at step 5 on `evaluated permission=external_directory patterns=["/tmp/*"]`, `message=asking`, `Error: The user rejected permission to use this specific tool call.` An absent deny entry means *unset*, and unset resolves to `ask`, which is fatal non-interactively. So on 1.17.11 the two levers contradict each other: the permission can only be granted through a `permission` block, and any `permission` block hangs the run. **The remaining lever is the brief** — see the `/tmp` rule under *Brief*.

Check `opencode --version` before reaching for the block, and diagnose with the ladder below rather than by assuming which failure you have: on `extract-test-fixtures` the tool-using smoke test hung, the tool-less one returned `OK` in seconds, and the *same* tool-using prompt with no config file succeeded and logged `action.action=allow`. That third step is what localises it to the config rather than to the permission model.
...
**Never tell the reviewer it may use `/tmp`.** "Use `/tmp` if you need scratch space" is the single sentence that killed the first `add-expiring-urls` run: it is an invitation to the `external_directory` abort above, and on 1.17.11 that permission cannot be granted. Say the opposite, explicitly — do not read or write outside the project directory, do not redirect command output to a file and read it back, page through `git diff main...HEAD` with `head`/`sed -n`/`grep` on the command's own stdout instead. Left unsaid, the reviewer invents the redirect on its own; the brief has to close the door rather than merely not open it.

**Brief** — always include: the artifact (`git diff main...HEAD` on the worktree), the `/tmp` prohibition above, hunting priorities in order, H/M/L severity on *every* finding, a **quoted snippet** per finding rather than a bare `file:line` (see below), "do not edit any files", the findings-as-final-message instruction above, and this project's hard rules so the reviewer does not fight them: no new dependencies (see *Dependency policy*), config is `AP_`-prefixed env vars only, errors are data not exceptions, ffmpeg args are argv lists never shell strings, and every option must round-trip to an identical cache key.
...
**When the change adds a guard, mutate the guard — do not read it.** A slice whose point is that something is now checked has a self-review question no amount of reading answers: does the suite fail when the check is gone? Delete the plug, negate the invariant, run the suite, restore. On `add-expiring-urls` six mutations took four minutes and two of them stayed green — the production pipeline could lose its expiry plug entirely, and the write-back could store the clamped policy, both with 1051/1051 passing. Both were the change's most load-bearing guarantees, and **the CLI reviewer read the same files and reported the opposite**, stating the tests "would fail if `CheckExpiry` were unmounted". A reviewer inferring coverage from a plug being visible in a test file is the failure mode; running the mutation is the only thing that distinguishes a guard from a decoration. Reserve it for what the change claims to enforce, not for every line it touches.
