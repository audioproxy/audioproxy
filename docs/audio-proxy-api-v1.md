...
| `sr` | Hz (`44100`, `48000`, …) | Resample. Omitted, nothing is resampled and the encoder gets the source's rate; an explicit value above 48 kHz is refused for lossy formats. See the note below for where the ceiling does and does not apply |
...
**"Default: the source's" means the probed source, not a stand-in.** Both defaults above are answered from the `ffprobe` gate below, which runs on every miss anyway. So `f:flac/norm:ebu` on a 96 kHz master renders at 96 kHz rather than downsampling — `norm` forces a resample, because single-pass `loudnorm` emits 192 kHz, and the rate it resamples *to* is the source's — and `f:wav` on a 24-bit master renders 24-bit. Where a source's rate or depth cannot be read, the fallbacks are 48 kHz and 16-bit.

**The 48 kHz lossy ceiling is a rule about `sr` and `norm`, not a cap on every lossy render.** It applies in exactly two places: an explicit `sr` above it is refused, and the rate `norm` resamples to is clamped to it, so `f:mp3/norm:ebu` on a 96 kHz master renders at 48 kHz. It does **not** apply to a plain lossy render, which emits no resampling stage at all and hands the encoder whatever the source had: `f:aac` or `f:m4a` on that same master really does return 96 kHz audio. `f:mp3` and `f:opus` return 48 kHz there, but that is their own encoders refusing higher rates rather than this proxy imposing one.
