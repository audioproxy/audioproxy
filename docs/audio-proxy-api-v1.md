...
Peaks respect `t`, `ch` and `fade`, ignore encoding options, and refuse `enhance` along with the loudness options: a waveform is drawn from the source's own shape, and the preset's compressor would reshape the envelope the picture describes. Cheap enough to render eagerly alongside any audio variant later, but v1 renders on request.
