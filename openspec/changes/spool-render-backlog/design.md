...
- **The mode flag is a rollout tool, not the end state.** A *mode* asks the operator a question they usually cannot answer — "will you ever serve long-form lossless?" — and punishes a wrong guess with an OOM rather than a degradation. The person most likely to answer wrong is exactly the one who has not read `docs/capacity.md`. The end state is therefore neither mode: **keep the backlog in memory up to a threshold and spill to a file past it**, so the knob is a size rather than a mode and the workload declares itself. Preview-shaped renders (a 30-second Opus is ~360 KB) never touch disk, keeping every advantage claimed for the memory path above; long-form stops being a memory bomb without anyone predicting it.

  That is deliberately *not* this change. Spilling is a third behaviour to test rather than a second — memory, spilled, and the transition — and the spill point is a new failure surface: a write that fails mid-stream has to kill that render rather than silently fall back, or subscribers diverge. This change ships the file path and proves it against the memory path; `spill-render-backlog` makes the choice automatic; a third change retires `AP_BACKLOG_MODE` once the threshold path has earned it. Recorded here so the mode question is not re-litigated when the trigger fires.
