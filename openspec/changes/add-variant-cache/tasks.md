...
- [ ] 1.1 `AudioProxy.VariantStore` behaviour: `head/1`, `get_stream/2` (Range-aware), `put_stream/2`, `presign/2`, `capabilities/0`; `@type t` and `@spec` on the seam, since it is a public contract per the typing convention
...
- [ ] 2.3 `get_stream/2` with Range: serve the requested slice without reading the whole file
- [ ] 2.4 Boot check: store directory exists and is writable, failing loudly as `AP_LOCAL_ROOT` does

## 3. Tee write-back

- [ ] 3.1 `VariantCache.tee/2` Task subscriber: consume coordinator stream → `put_stream`; abort on error/cancel; log and instrument write failure without touching client streams
...
- [ ] 4.2 Proxy mode: streamed passthrough with Range/206/`Accept-Ranges`/`Content-Length`
...
- [ ] 5.2 Backend parity suite: run the same behaviour tests against both backends
...
- [ ] 6.5 Slow-client full-speed render: throttled reader, assert the variant is complete before the client finishes
- [ ] 6.6 Disconnect policy: cache on → sole client disconnect still completes write-back; cache off → cancels
- [ ] 6.7 Config: `redirect` + `file://` store aborts boot; unknown scheme aborts boot
