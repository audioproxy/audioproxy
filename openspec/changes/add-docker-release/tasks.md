...
- [ ] 3.1 Publish job in `ci.yml` (`needs: smoke`): GHCR login via `GITHUB_TOKEN`; on `v*` tags push `:X.Y.Z`/`:X.Y`/`:latest`, on `main` push `:edge` + `:sha-<12>`
- [ ] 3.2 Release assertion: `mix.exs` version == git tag, or the publish job fails
...
- [ ] 4.1 Update README: docker pull/run quickstart (local-source mode, zero S3 config; recommend `:X.Y.Z` or `:sha-*`, not `:latest`), env reference
- [ ] 4.2 `docs/development.md`: release procedure (tag → publish), versioning policy (what bumps what — incl. "pin bumps cut a patch release" and "cache-key changes are major"), upgrade procedure for pinned toolchain versions
