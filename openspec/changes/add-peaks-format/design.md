...
- **Bucket boundaries from total sample count**, which requires knowing duration up front: take it from a leading ffprobe (already built) rather than buffering; `samples_per_pixel = ceil(region_samples / pts)`, last bucket may be partial. Duration drift between probe and decode handled by clamping (spec tolerance).
