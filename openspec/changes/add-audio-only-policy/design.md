...
- **Protocol whitelist as builder baseline**, set derived from config: `https,tls,tcp`, plus `http` automatically when the S3 endpoint override is plaintext (dev/MinIO). Not user-configurable beyond that — an env knob for protocols would reopen the hole this closes.
...
- [Extra ffprobe per MISS adds latency] → header-read only (hundreds of ms worst case on cold S3), amortized to zero by the variant cache; acceptable for the security/abuse win.
