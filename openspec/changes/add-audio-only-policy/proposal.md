...
- **Input protocols restricted**: ffmpeg runs with `-protocol_whitelist` limited to `https,tls,tcp` (plus `http` only when an `AP_S3_ENDPOINT`-style dev override demands it), so even a compromised or redirecting source cannot pivot ffmpeg to local files or other protocols.
...
- `ffmpeg-args`: Every render argv SHALL disable video/subtitle/data streams and restrict input protocols (delta on the command-builder spec).

## Impact

- Modified: `lib/audio_proxy/ffmpeg/command.ex` (stream-disable flags, protocol whitelist, allowlist introspection for tests), render action (probe gate before subscribing), `docs/audio-proxy-api-v1.md`.
- Depends on: `add-ffmpeg-command-builder`, `add-render-endpoint`, `add-info-endpoint` (reuses its `Ffprobe` module for the gate).
