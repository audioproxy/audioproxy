...
- `mix.exs` package metadata: `description`, `package` (licenses, GitHub link, curated `files:` list — `lib`, `mix.exs`, `README.md`, `LICENSE`, `VERSIONS.md`, `docs/`; never `openspec/`, `test/`, `examples/`, `Dockerfile`, `.github/`), `docs` config; `{:ex_doc, only: :dev, runtime: false}`.
