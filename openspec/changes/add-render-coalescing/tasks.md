...
- [ ] 2.4 Lifecycle: kill one subscriber (others complete), kill all (subprocess cancelled), mid-render error (all notified, key retryable)
...
assertions) moved to `add-render-semaphore`, which is post-MVP — the MVP
coordinator spawns its pipeline directly.
