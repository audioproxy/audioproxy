...
- [x] 1.1 `VariantStore.S3`: `head/1` via `S3.head/2`, `get_stream/2` via `S3.get_stream/3` (range through), `put_stream/3` via `S3.put_stream/4`
- [x] 1.2 Metadata onto the object — `content_type` and `cache_control` as real headers, the rest as `x-amz-meta-*` — and read back off the HEAD in `head/1`
- [x] 1.3 `presign/2` via `S3.presign_get/3` with `AP_PRESIGN_TTL`; `capabilities/0` returns `[:presign]`
- [x] 1.4 Classify this layer's failures per `add-s3-source-backend`'s table, reusing it rather than restating it

## 2. Configuration

- [x] 2.1 `Config`: accept `s3://bucket`, replacing the raise; reject a path, query or fragment on the URL the way the `file://` branch does
- [x] 2.2 Boot-time writability probe — a small PUT and DELETE under a reserved key prefix — so a bucket that refuses writes fails the container rather than silently discarding write-backs
- [x] 2.3 Confirm `validate!/1` now admits `AP_SERVE_MODE=redirect` with an `s3://` store, and still refuses it with `file://`, without changes to the validator
