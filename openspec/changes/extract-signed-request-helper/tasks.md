...
- [x] 2.7 Convert all 18 `put_config` calls to `base_config/1` plus overrides, checking each against the 1.2 record key by key
  - **15** files call `base_config/1`, `source/s3_backend_test.exs` among them (see 4.4 for the `local_root: nil` it needed to get there). The other four carry key material without the floor and keep their own map, now calling `key/0` and `salt/0`: `signature_test.exs`, both `verify_signature` files (`%{key:, salt:, allow_insecure:}` only), and `variant_cache_s3_test.exs`, whose config goes through `MinioHelper.configure!/1` rather than `put_config/1`. 15 + 4 = the 19 files that carried the preamble.
