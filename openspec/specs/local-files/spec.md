# local-files Specification

## Purpose
Serving audio off a mounted directory, which is what lets the proxy render
anything at all without an object store: a path handoff and a `stat` replace a
presigned URL and a HEAD. One configured root (`AP_LOCAL_ROOT`) is the whole
access-control story for disk — unset means local sources are refused, so a
deployment serves exactly what it chose to mount.

The rest is confinement, and it is defined over *paths*: a resolved path must
still sit under the resolved root once every symlink on it has been followed,
and every failure is the same 404 as a missing file, so the root cannot be used
to probe the filesystem around it. What that leaves uncovered — hardlinks, and
the window between resolving a path and opening it — is a deployment
assumption, discharged by mounting the root read-only.
