...
## Purpose
What an operator can measure about a running node, as distinct from what the
log says happened. `request-logging` narrates individual requests; this
aggregates them into the four signals that decide whether to scale — render
saturation, render latency, cache efficiency, and error rate — and exposes
them to any Prometheus-compatible scraper.

The instrumentation underneath is the telemetry `request-logging` already
established, with one event added for the cache outcome that no render event
could carry: a HIT never starts a render. Instrument once, consume twice.

The endpoint is unsigned, so its reach is a bind address rather than a
signature — a guarantee the kernel makes, chosen over a peer-address check
this code would have to make behind a proxy that rewrites it.
