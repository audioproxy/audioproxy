...
## Purpose
What the proxy says about itself while it runs. One line per completed HTTP
request — request id, endpoint class, normalized options, canonical source,
status, duration, bytes — at a level chosen by outcome, so a client error
reads as routine and a server failure stands out.

The instrumentation underneath is telemetry rather than log calls: the render
path emits lifecycle events and the log handler is merely their first
consumer, so the metrics endpoint attaches an aggregator later to events that
already exist. Instrument once, consume twice.

One guarantee is load-bearing rather than incidental. Render inputs reach
ffmpeg as presigned URLs once remote sources land, and a log line carrying one
would put a fetchable secret into log storage. Sources are therefore logged by
their canonical identity only, and diagnostics quoted back from ffmpeg are
redacted — the guarantee holds on every path, including the failure paths that
exist to explain what went wrong.
