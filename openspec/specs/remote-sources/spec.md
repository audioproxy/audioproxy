# remote-sources Specification

## Purpose
The two source forms that name something the proxy does not already hold: an
object in a bucket (`s3://`) and a URL at an origin (`https://`). Both are
remote, so both are gated — one allowlist, `AP_SOURCE_ALLOWLIST`, because the
question is the same one for each and only the spelling of a namespace differs.

The gate is asymmetric by design. A bucket prefix glob hands out nothing,
because that namespace belongs to the operator; a host takes a label-anchored
suffix glob, because anyone with a registrar can create a name that merely
*starts* the same way. Unset accepts S3 and refuses HTTPS: bucket credentials
are already a gate, and an ungated outbound fetch is a server-side request
forgery primitive.

The rest is identity. A remote source has to fold every second spelling of one
resource onto one canonical string — case, a root dot, a default port, an empty
query — or one object quietly wears several cache keys; and it has to leave
alone what only the origin can resolve, its own percent-encoding and its dot
segments, or two objects share one. What a source parses to and what the gate
authorizes must be the same host, always.
...
### Requirement: An HTTPS host is validated after it is normalized
The system SHALL reject an `https://` source whose host, once lowercased and stripped of a single trailing root dot, is empty, contains an empty label, or contains a percent-escape, and SHALL derive the host the allowlist is applied to by the same normalization, so that parsing and authorization never disagree about which host a URL names. IP literals are exempt from the label rules.

#### Scenario: Dot-only host refused
- **WHEN** the decoded source is `https://./a.wav` or `https://.../a.wav`
- **THEN** parsing fails with a structured error, rather than yielding a canonical string with no host

#### Scenario: Empty label refused
- **WHEN** the decoded source is `https://cdn..media.example/a.wav`
- **THEN** parsing fails, so an allowlist entry of `*.media.example` cannot admit unboundedly many spellings of one resource

#### Scenario: Percent-escaped host refused
- **WHEN** the decoded source is `https://%6D%65dia.example/a.wav`
- **THEN** parsing fails, because the escape is not decoded and would otherwise reach the canonical identity

#### Scenario: Authorization agrees with parsing
- **WHEN** a source parses and the allowlist is a bare `*`
- **THEN** authorization succeeds, and re-parsing the canonical string yields an identical source
